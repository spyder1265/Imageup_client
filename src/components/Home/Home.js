        ...
        <div className="flex relative flex-col h-screen w-screen max-h-screen bg-slate-300 ">
            ...
            <div className="min-w-full bg-transparent basis-12">
                <div className="px-6 py-4">
                    <label className="block text-gray-700 font-bold mb-2">Select a file:</label>
                    <input type="file" accept="image/*,gif" onChange={handleFileChange} />
                    {selectedFile && (
                        <>
                            <div className="mt-4">
                                <span className="text-gray-700 font-bold">File name:</span> {selectedFile.name}
                            </div>
                            <div className="mt-2">
                                <span className="text-gray-700 font-bold">File type:</span> {selectedFile.type}
                            </div>
                            <div className="mt-2">
                                <span className="text-gray-700 font-bold">File size:</span> {selectedFile.size} bytes
                            </div>
                        </>
                    )}
                    {error && (
                        <div className="mt-4">
                            <span className="text-red-500">Only image and gif files are allowed.</span>
                        </div>
                    )}
                    <div className="mt-4">
                        <button
                        ...
                        >
                            <div className="mt-2">
                                ...
                                <div className="text-gray-700 text-xs mt-2">{uploadProgress}%</div>
                            </div>
                        </Transition>
                        <Transition
                            show={success}
                        ...
                        >
                            <div className="mt-4 flex items-center">
                                ...
                                <span className="text-gray-700 font-bold">File uploaded successfully!</span>
                            </div>
                        </Transition>
                    </div>
                </div>
            ...
            <div className="flex w-screen absolute bottom-0  flex justify-center bg-black xl:basis-[50%]">
                <div className="grid  auto-cols-max grid-flow-col overflow-y-auto gap-x-16 mx-8 px-12 py-12 h-full w-full place-items-center  ">
                    {images.reverse().map((image,index)=>(
                            <div key={index} className="bg-white border border-white relative group laptop:w-[350px] laptop:h-[350px]  phone:w-[200px] phone:h-[200px] rounded-lg h-80" >
